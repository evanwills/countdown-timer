...
class TalkingTimer extends HTMLElement {
  ...
  parseRawIntervals (rawIntervals, durationMilli, omit) {
    ...
    while ((matches = regex.exec(rawIntervals)) !== null) {
      let interval = {
        ...
        multiplier: (typeof (matches[2] * 1) === 'number') ? parseInt(matches[2]) : 1,
      ...
      if (typeof matches[7] !== 'undefined') {
        ...
        interval.offsets = getFractionOffsetAndMessage(interval, durationMilli)
        fractionIntervals.push(interval)
      } else {
        interval.isFraction = false
        ...
        interval.offsets = getTimeOffsetAndMessage(interval)
        timeIntervals.push(interval)
      }
    }

    return (this.priority === 'time') ? [ ...fractionIntervals, ...timeIntervals ] : [ ...timeIntervals, ...fractionIntervals ]
  }

  getIntervalPromises (durationMilli, remainingMilliseconds, intervals) {
  ...
  getFractionOffsetAndMessage (timeObj, milliseconds) {
    ...
    switch (timeObj.numerator) {
      case 2:
        return [{ message: 'Half way', offset: half }]
      case 3:
        message = 'third'
        break
      case 4:
        message = 'quarter'
        special = ['', 'one quarter', 'half way', 'three quarters']
        break
      case 5:
        message = 'fifth'
        break
      case 6:
        message = 'sixth'
        special = ['', 'one sixth', 'one third', 'half way', 'two third', 'five sixths']
        break
      case 7:
        message = 'seventh'
        break
      case 8:
        message = 'eighth'
        special = ['', 'one eighth', 'one quarter', 'three eighths', 'half way', 'five eighths', 'three quarters', 'seven eighths']
        break
      case 9:
        message = 'ninth'
        special = ['', 'one ninth', 'two ninths', 'one third', 'four ninth', 'five ninths', 'two thirds', 'seven ninth', 'eight ninths']
        break
      case 10:
        message = 'thenth'
        special = ['', 'one tenth', 'one fifth', 'three tenths', 'two fifths']
        break
    }
    ...
    if (timeObj.relative === 'first') {
      for (let a = 1; a <= count; a += 1) {
        const tmp = getSpecialMsg(message, special, a)
        offsets.push({
          offset: (milliseconds - (interval * a)),
          message: a + ' ' + tmp + ' gone.'
        })
      }
    } else if (timeObj.relative === 'last') {
      for (let a = 1; a <= count; a += 1) {
        const tmp = getSpecialMsg(message, special, a)
        offsets.push({
          offset: (interval * a),
    ...
    return offsets.map(item => {
      if (item.offset > (half - 5000) && item.offset < (half + 5000)) {
        return {
          offset: half,
          message: 'halfway'
        }
      ...
      if (item.message === 'halfway') {
        halfwayCount += 1
        return (halfwayCount <= 1)
      } else {
        return true
      }
    })
  ...
  getTimeOffsetAndMessage (timeObj, milliseconds) {
    return []
  }
   ...
   */
  getSpecialMsg (message, special, index) {
    if (special.length === 0) {
      const s = (index > 1) ? 's' : ''
